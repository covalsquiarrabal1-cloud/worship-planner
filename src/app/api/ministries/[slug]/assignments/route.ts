...
export async function POST(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  const { eventId, memberId, celebrationNumber, roleName } = await request.json()
  if (!eventId || !memberId) return NextResponse.json({ error: 'eventId e memberId obrigatórios' }, { status: 400 })
  ...
  const { data, error } = await serviceClient
    ...
    .insert({
      event_id: eventId,
      member_id: memberId,
      celebration_number: celebrationNumber || 1,
      role: 'operator',
      role_name: roleName || 'Membro',
    })
    .select()
    .single()
