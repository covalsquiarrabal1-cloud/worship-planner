...
export default function GerarEscalaMinisterioPage() {
  ...
  async function generate() {
    ...
    if (membersCount === 0) {
      alert('Nenhum membro cadastrado neste ministério. Adicione membros antes de gerar.')
      return
    }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5">
      ...
      <div className="flex items-center gap-3">
        <Link href={`/admin/ministerios/${slug}`} className="p-2 rounded-xl bg-[#1c2128] border border-[#30363d]">
      ...
      {membersCount !== null && (
        <div className={`card text-sm ${membersCount === 0 ? 'border-[#f85149]/40' : ''}`}>
          {membersCount === 0 ? (
            <p className="text-[#f85149]">⚠️ Nenhum membro cadastrado. <Link href={`/admin/ministerios/${slug}`} className="underline">Adicione membros</Link> antes de gerar.</p>
          ) : (
            <p className="text-[var(--muted-foreground)]">👥 {membersCount} membro{membersCount > 1 ? 's' : ''} disponíve{membersCount > 1 ? 'is' : 'l'} para escalar.</p>
          )}
        </div>
      )}
      ...
      {days.length === 0 ? (
        <div className="card text-center py-8">
          ...
          <p className="text-xs text-[var(--muted-foreground)] mt-1">
            Gere a escala do louvor primeiro.
          </p>
        </div>
      ) : (
        <>
