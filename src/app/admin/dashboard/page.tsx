...
export default function DashboardPage() {
  ...
  return (
    <div className="max-w-md mx-auto px-4 pb-8">
      ...
      {/* 2x2 Grid - vertical rectangular cards */}
      <div className="grid grid-cols-2 gap-4 relative">
        ...
        {cards.map((card) => (
          ...
          >
            {/* Flowing border - light traveling along the edge */}
                ...
                <div className="space-y-1.5">
                  {(() => {
                    ...
                    return people.length === 0 ? (
                      ...
                      <div key={idx} className="py-2.5 rounded-xl hover:bg-[var(--accent)] transition-colors" style={{ paddingLeft: '24px', paddingRight: '24px' }}>
                        <span className="text-sm font-medium">{person.name}</span>
                        {person.ministries.length > 0 && (
                          <p className="text-[10px] text-[var(--muted-foreground)] mt-0.5">{person.ministries.join(', ')}</p>
                        )}
